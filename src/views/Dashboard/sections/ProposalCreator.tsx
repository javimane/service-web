...
import { getFileSignedUrl, getProposalSignedUrl, uploadProposalPdf } from "../../../services/storageUploads";
...
export default function ProposalCreator({ onBack }) {
  ...
  const professionalAddress = useMemo(() => {
    ...
    const mainAddress = actualCompany?.address || (Array.isArray(myProfessional?.address) ? myProfessional?.address.find((a: any) => a.is_main_address) : myProfessional?.address);
  ...
  const filteredClients = useMemo(() => {
    ...
    return chatClients.filter((client: any) => 
      client.display_name?.toLowerCase().includes(clientSearch.toLowerCase()) ||
  ...
  // Calculations
  const itemsSum = useMemo(() => items.reduce((acc, item) => acc + item.total, 0), [items]);
  ...
  const generatePdfBlob = async () => {
    ...
    let textX = 40;
    const professional = {
      name: profileData?.display_name || user?.user_metadata?.full_name || user?.display_name || "Profesional",
      email: user?.email || "",
      address: professionalAddress || "",
      companyName: professionalCompany?.name || "",
      avatarUrl: profileData?.avatar_url || user?.user_metadata?.avatar_url || user?.avatar_url || ""
    };
    ...
    const startY = 180;
    const client = {
      name: selectedClient ? (selectedClient.display_name || "") : clientSearch,
    ...
    if (hasTax) {
      ...
      doc.text(`${currencySymbol} ${subtotal.toLocaleString(undefined, { minimumFractionDigits: 2 })}`, pageWidth - 50, finalY + 17, { align: "right" });

      const taxPercent = subtotal > 0 ? Math.round((tax / subtotal) * 1000) / 10 : 0;
      doc.text(`IVA (${taxPercent}%)`, pageWidth - 210, finalY + 34);
      doc.text(`${currencySymbol} ${tax.toLocaleString(undefined, { minimumFractionDigits: 2 })}`, pageWidth - 50, finalY + 34, { align: "right" });
      ...
      doc.text(`${currencySymbol} ${total.toLocaleString(undefined, { minimumFractionDigits: 2 })}`, pageWidth - 50, finalY + 57, { align: "right" });
    } else {
      doc.setFont("helvetica", "bold");
    ...
    doc.text(`Dirección: ${professional.address || "Sin Dirección"}`, 40, pageHeight - 40);
    doc.text(`Correo: ${professional.email || "Sin Email"}`, pageWidth / 2 - 50, pageHeight - 40);
  ...
  const handleSend = async () => {
    if (!selectedClient) {
      alert("Por favor, selecciona un cliente de la lista para poder registrar y enviar el presupuesto.");
      return;
    }
    ...
    try {
      ...
      const res = await createProposalAction({
        data: {
          ...
          accepted: false,
          professional_name: profileData?.display_name || user?.user_metadata?.full_name || user?.display_name || "Profesional"
        },
      ...
      alert("¡Presupuesto creado y guardado con éxito! Redirigiendo a tus mensajes...");
      ...
      alert(error.message || "Ocurrió un error al procesar el presupuesto. Intentá nuevamente.");
    } finally {
      setIsSending(false);
    }
  };
    ...
    <div className="proposal-creator">
      <header className="proposal-header">
        ...
        <div className="header-actions">
          <button className="btn-primary" onClick={() => setIsPreviewOpen(true)}>Previsualizar</button>
        </div>
      </header>
          ...
          <section className="proposal-card step-card">
            ...
            <div className="form-group">
              ...
              <div className="search-input-wrapper">
                ...
                <input 
                  type="text" 
                  placeholder="Buscar o añadir nuevo cliente..." 
                  value={clientSearch}
                  onChange={(e) => {
              ...
              <button 
                ...
                onClick={() => {
                  alert("Próximamente: Añadir un nuevo perfil de cliente.");
                  setShowClientDropdown(false);
                }}
              >
                <Plus size={14} /> AÑADIR NUEVO PERFIL DE CLIENTE
              </button>
            </div>
          </section>
            ...
            <div className="form-row proposal-form-row">
              <div className="form-group">
                ...
                <div className="select-input-wrapper">
                  <select 
                    value={taxMethod} 
                    onChange={(e) => setTaxMethod(e.target.value as 'added' | 'included')}
                    className="settings-select"
                  >
                    <option value="added">Sumar al total (Precio + IVA)</option>
                    <option value="included">Incluido en el total (Discriminar)</option>
                  </select>
                  <div className="dropdown-arrow">▼</div>
                </div>
              </div>
        ...
        <div className="proposal-col">
          <section className="proposal-card items-card">
            ...
            <div className="items-list-container">
              {items.length === 0 ? (
                <div className="empty-items">Aún no se han añadido ítems. Haz clic en "+ AÑADIR" para comenzar.</div>
              ) : (
                  ...
                  {items.map((item) => (
                    <div key={item.id} className="item-row">
                      ...
                      <div className="item-info">
                        <span className="item-label">DESCRIPCIÓN / NOMBRE DEL ÍTEM</span>
                      ...
                      <div className="item-metrics">
                        ...
                        <div className="metric">
                          <span className="item-label">UNITARIO ({currencySymbol})</span>
                          <p>{currencySymbol} {item.rate}</p>
                        </div>
                        <div className="metric total">
                          <span className="item-label">TOTAL</span>
                          <p>{currencySymbol} {item.total.toLocaleString()}</p>
                        </div>
                      </div>
                    </div>
                  ))}
      ...
      <section className="proposal-summary-card">
        <div className="summary-info">
          ...
          <p>Todos los cálculos de subtotal, impuestos e importes totales se generan automáticamente.</p>
          <div className="summary-badges">
            <div className="summary-badge">
              <span className="badge-label">MONEDA</span>
              <div className="badge-value">{currency === 'ARS' ? 'Pesos ($)' : 'Dólares (USD)'}</div>
            </div>
          </div>
        </div>
        ...
        <div className="summary-values">
          ...
          <div className="value-row">
            <span>IVA ({taxRate * 100}%) {taxMethod === 'included' ? '(Incluido)' : ''}</span>
      ...
      <footer className="proposal-footer">
        ...
        <div className="footer-btns">
          ...
          <button className="btn-send" onClick={handleSend} disabled={isSending}>
            <Send size={18} /> {isSending ? "ENVIANDO..." : "ENVIAR PRESUPUESTO"}
          </button>
        </div>
      </footer>
