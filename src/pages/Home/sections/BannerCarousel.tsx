...
import { ChevronLeft, ChevronRight, Users, Store, Star, ArrowRight } from "lucide-react";
...
export default function BannerCarousel() {
  ...
  const handleAction = (banner: typeof BANNERS[0]) => {
    if (banner.type === "referrals") {
      if (user) {
        navigate(ROUTES.dashboard, { state: { view: "referrals" } });
      } else {
        openAuth("login");
      }
  ...
  return (
    <section className="banner-carousel">
                ...
                <div className="banner-slide__content">
                  <div className="banner-slide__info">
                    ...
                    <div className="banner-slide__text-group">
                      <h3>{banner.title} {banner.highlight && <span className="banner-slide__highlight">{banner.highlight}</span>}</h3>
                      <p>{banner.text}</p>
                    </div>
                  </div>
                  <button 
      ...
      <button className="banner-carousel__nav banner-carousel__nav--prev" onClick={prev}>
        <ChevronLeft size={24} />
      </button>
      <button className="banner-carousel__nav banner-carousel__nav--next" onClick={next}>
