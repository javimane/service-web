...
export default function HeadquartersSection({
  ...
  const handleSelectAllProvinces = () => {
    ...
    const someMissing = allNames.some(name => !selectedProvinces.includes(name));
  ...
  const handleSelectAllDepartments = () => {
    ...
    const someMissing = allNames.some(name => !selectedDepartments.includes(name));
      ...
      <div className="geo-selector">
        ...
        <div className="geo-column geo-column--provinces">
          <div className="geo-column__header">
            <h3>Provincias</h3>
            <button type="button" className="geo-select-all" onClick={handleSelectAllProvinces}>
              {provinceList.length === selectedProvinces.length ? "Deseleccionar todas" : "Seleccionar todas"}
            </button>
          </div>
          <div className="geo-list">
            {provinceList.map((p) => (
              ...
              >
                <label className="geo-checkbox" onClick={e => e.stopPropagation()}>
                  <input 
                    type="checkbox" 
                    checked={selectedProvinces.includes(p.name)}
                    onChange={() => onToggleProvince(p.name)}
                  />
                  <div className="checkbox-box"><Check size={12} /></div>
                </label>
        ...
        <div className="geo-column geo-column--departments">
          <div className="geo-column__header">
            <h3>Departamentos</h3>
            {activeProvince && (
               <button type="button" className="geo-select-all" onClick={handleSelectAllDepartments}>
                {filteredDepartments.length > 0 && filteredDepartments.every(d => selectedDepartments.includes(d.name)) 
                  ? "Deseleccionar todos" 
                  : "Seleccionar todos"}
               </button>
            )}
