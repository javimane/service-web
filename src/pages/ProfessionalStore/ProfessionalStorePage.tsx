...
import {
  Search, Filter, X, Loader2, Package,
  Star, Grid3X3, List, SlidersHorizontal,
  Globe, ArrowLeft, ShoppingBag,
...
export default function ProfessionalStorePage() {
  ...
  const productsList = useMemo(() => {
    return normalized.data.map((item: any) => {
      ...
      return {
        ...
        image: primaryImage,
        images: sorted.map((i: any) => i.image_url).concat(item.image_url ? [item.image_url] : []),
        price: displayPrice,
        offerPrice,
  ...
  const storeName = company?.name || profile?.display_name || "Tienda Profesional";
  const avatar =
    profile?.avatar_url ||
    `https://ui-avatars.com/api/?name=${encodeURIComponent(storeName)}&background=e94823&color=fff&size=128`;
  const bannerImage = profile?.portfolio_image_url || profile?.banner_url || null;
    ...
    return (
      <div className="store-loading-screen">
        ...
        <div className="store-loading-content">
          ...
          <button onClick={() => navigate(-1)} className="store-back-btn">
    ...
    <div className="professional-store">
      ...
      >
        ...
        <div className="store-banner__content">
          <button className="store-banner__back" onClick={() => navigate(`/profile/${id}`)}>
            <ArrowLeft size={16} /> Volver al perfil
          </button>
          <div className="store-banner__identity">
            <div className="store-banner__avatar-wrap">
              <img src={avatar} alt={storeName} className="store-banner__avatar" />
            </div>
            <div className="store-banner__text">
              ...
              <p className="store-banner__meta">
                {normalized.count} producto{normalized.count !== 1 ? "s" : ""} disponibles
              </p>
            </div>
          </div>
        </div>
      ...
      <main className="store-main">
        ...
        <div className="store-toolbar">
          ...
          <div className="store-toolbar__right">
            ...
            <button className={`store-filter-btn ${showFilters ? "active" : ""}`} onClick={() => setShowFilters(!showFilters)}>
              <Filter size={16} /> Filtros
              {activeFiltersCount > 0 && <span className="store-filter-badge">{activeFiltersCount}</span>}
            </button>
          </div>
        </div>
        ...
        <div className="store-layout">
          ...
          <aside className={`store-sidebar ${showFilters ? "open" : ""}`}>
            ...
            <div className="store-filter-section">
              <label><SlidersHorizontal size={14} /> Precio</label>
              <div className="store-filter-range">
                <input type="number" min="0" placeholder="Mín" value={filters.priceMin}
                  onChange={(e) => handleFilterChange("priceMin", e.target.value)} className="store-filter-input" />
                <span>—</span>
                <input type="number" min="0" placeholder="Máx" value={filters.priceMax}
                  onChange={(e) => handleFilterChange("priceMax", e.target.value)} className="store-filter-input" />
              </div>
            </div>

            <div className="store-filter-section">
              <label>EAN / Código</label>
              <input type="text" placeholder="Ej: 7791234567890" value={filters.ean}
                onChange={(e) => handleFilterChange("ean", e.target.value.replace(/\D/g, ""))}
                inputMode="numeric" className="store-filter-input" />
            </div>

            <button className="store-clear-btn" onClick={clearFilters}>Limpiar filtros</button>
          </aside>

          {/* Products */}
          <section className="store-results">
            <p className="store-count">
              {loadingProducts ? "Buscando..." : `${normalized.count} producto${normalized.count !== 1 ? "s" : ""}`}
            </p>
            ...
            {loadingProducts ? (
              ...
              <div className="store-empty">
                ...
                <button className="store-clear-btn" onClick={clearFilters}>Ver todos</button>
              </div>
            ) : (
                ...
                {productsList.map((product) => (
                  ...
                  >
                    <div className="store-product-card__image">
                      ...
                      {product.is_foreign && (
                        <span className="store-badge store-badge--foreign"><Globe size={10} /> EXTERNO</span>
                      )}
                      {product.discount > 0 && (
                        <span className="store-badge store-badge--discount">-{product.discount}%</span>
                      )}
                    </div>
                    <div className="store-product-card__body">
                      <h3 className="store-product-card__title">{product.title}</h3>
                      <div className="store-product-card__pricing">
                        {product.offerPrice ? (
                          <>
                            <span className="store-price--original">${formatPrice(product.price)}</span>
                            <div className="store-price--row">
                              <span className="store-price--main">${formatPrice(product.offerPrice)}</span>
                              {product.discount > 0 && (
                                <span className="store-price--off">{product.discount}% OFF</span>
                              )}
                            </div>
                          </>
                        ) : (
                          <div className="store-price--row">
                            <span className="store-price--main">${formatPrice(product.price)}</span>
                          </div>
                        )}
                      </div>
            ...
            {!loadingProducts && normalized.totalPages > 1 && (
              <div className="store-pagination">
                <button className="store-pagination__btn" onClick={() => setPage((p) => Math.max(p - 1, 1))} disabled={page <= 1}>
                  <ChevronLeft size={16} /> Anterior
                </button>
                <span className="store-pagination__info">Página {page} de {normalized.totalPages}</span>
                <button className="store-pagination__btn" onClick={() => setPage((p) => Math.min(p + 1, normalized.totalPages))} disabled={page >= normalized.totalPages}>
