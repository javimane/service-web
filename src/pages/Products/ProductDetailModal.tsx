        ...
        <div className="product-modal__layout">
          ...
          <div className="product-modal__right-column">
            <div className="product-modal__sellers-container">
              <h3 className="sellers-title">Disponible en {sellers.length} tiendas</h3>
              
              <div className="sellers-list">
                {sellers.map((item: any, idx: number) => {
                  const companyData = item.Professional?.Company;
                  const sellerName = Array.isArray(companyData) 
                    ? companyData[0]?.name 
                    : companyData?.name || "Profesional independiente";

                  const addressData = item.Professional?.address;
                  const sellerProvince = Array.isArray(addressData)
                    ? addressData[0]?.Province?.name
                    : addressData?.Province?.name || "Ubicación no especificada";

                  const professionalId = item.Professional?.id || item.professional_id;
                  const userId = item.Professional?.user_id || professionalId;

                  // Determine Prices
                  const hasDiscount = item.offer_price || item.discount_percentage > 0;
                    ...
                    <div key={item.id || idx} className="seller-card">
                        <div className="seller-card__header">
                          ...
                          <div className="seller-card__info">
                            ...
                            <Link to={`/profile/${userId}`} onClick={onClose} className="seller-name">
                              {sellerName}
                            </Link>
                          </div>
                        </div>
                          ...
                          <div className="prices">
                            ...
                            <div className="seller-current-price-row">
                              <span className="seller-price">
                                ${formatPrice(hasDiscount && offerPrice ? offerPrice : originalPrice)}
                              </span>
